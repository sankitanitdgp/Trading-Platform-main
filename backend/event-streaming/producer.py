...
if __name__=="__main__":
    ...
    schedule.every(1).day.do(updatePortfolio)
    ...
    if datetime.datetime.now(timezone(timeZone)).time()>datetime.time(16,0,0) or datetime.datetime.now(timezone(timeZone)).time()<datetime.time(9,30,0):
        ...
        schedule.every(10).seconds.do(kafka_producer_fake,producer,'NFLX')
        schedule.every(10).seconds.do(kafka_producer_fake,producer,'GOOGL')
        schedule.every(10).seconds.do(kafka_producer_fake,producer,'WFC')
        schedule.every(10).seconds.do(kafka_producer_fake,producer,'MSFT')
    else:
